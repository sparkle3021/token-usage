...
export function useDashboardData() {
  ...
  const setData = useCallback((data, hourRows, todayEvents) => {
    setDaily(data.daily || []);
    setHour(hourRows || []);
    setTime(todayEvents || []);
    setLoaded(true);
  ...
  const fetchData = useCallback((silent, days) => {
    ...
    // 本次调用抢占的序号：任何后续用户操作/新刷新都会推高 fetchIdRef.current 使本次调用整体作废。
    const id = ++fetchIdRef.current;
    return Promise.all([
      api.getDashboardData(),
      api.getHourSeries(days === undefined ? 90 : days),
      api.getTodayEvents(),
    ])
      .then(([data, hourRows, todayEvents]) => {
        if (id !== fetchIdRef.current) return; // 过期响应丢弃
        setData(data, hourRows, todayEvents);
      })
      .catch(err => { if (id === fetchIdRef.current) setLoadError(String(err)); })
      .finally(() => { if (!silent) setRefreshing(false); });
  }, [setData]);

  // 切时间范围异步补拉：仅重拉该范围小时序列；time（今天原始事件）恒定，不被范围切换覆盖或清空
  const fetchTimeSeries = useCallback((days) => {
    const id = ++fetchIdRef.current;
    api.getHourSeries(days)
      .then(hourRows => {
  ...
  const M = useMemo(() => {
    ...
    return { daily, time, hour, today: daysAgo(0) };
  }, [loaded, daily, time, hour]);
