...
func (m *Manager) bulkUpsertSessionExec(ex preparedExecer, rows []model.SessionUsage) error {
	if len(rows) == 0 {
		return nil
	}
	return bulkExecPrepared(ex, rows, `
		INSERT INTO session_usage (device,source,session_id,last_activity,project_path,
		...
		func(r model.SessionUsage) []interface{} {
			return []interface{}{r.Device, r.Source, r.SessionID,
